...
| `tests/director.ts` | acceptance | the "discrete executive" spec: a ~6-operator tactical demo domain (GoTo/Breach/TakeCover/Suppress/Regroup/Idle, in `tests/director-fixture.ts`) with live-query external preconditions and an external cost oracle — correct plans, improve/repair replans, every `PlanEventStream` failure cause as structured data, multi-agent stream merge, and a <10ms replan perf gate |
