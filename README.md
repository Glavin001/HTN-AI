...
[`scenarios/squad-combat.ts`](./scenarios/squad-combat.ts) is a vertical slice that drives this engine as **game AI** — the use case [F.E.A.R.](https://en.wikipedia.org/wiki/F.E.A.R._(video_game)) made famous, where the *illusion of intelligence* came not from a fancy per-agent planner but from **squad coordination** layered over modest GOAP. Here every NPC runs the real reactive `Planner` (one `Model` + `ExecState` each — the ExecState *is* that unit's belief/working memory) over a shared world + coordination blackboard, with a perception step (line-of-sight + hearing + memory decay) producing the dirty writes that drive fluent-precise reactive replanning. No core changes — it's all built from the existing extension points.
